...
def decrypt_connection_string(encrypted_password, token, iv_str):
...
class Dashboardconnection(Resource):
    ...
    def post(self):
        ...
        encrypted_string = data.get('maskedConnection')
        token=data.get('token')
        iv=data.get('iv')
        connection_uris=decrypt_connection_string(encrypted_string,token,iv)
        ...
        try:
            if db_type == "mysql":
                ...
                return execute_mongo_query(query, connection_uri,connection_uris)
                 ...
                 {"error": "Unsupported database type"}, 400
